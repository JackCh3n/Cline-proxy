...
- **多平台 CI/CD**：GitHub Actions 自动构建，推送代码到 main 即触发，生成 Windows (amd64/arm64)、Linux (amd64/arm64)、macOS (amd64/arm64) 共 6 个平台二进制，发布标题格式为 `v年_月日`（如 `v2026_0807`）
...
Release 版本号格式为 `v年_月日`（如 `v2026_0807`），每次推送自动生成，标题与标签保持一致。
